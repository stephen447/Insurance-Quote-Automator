"""An Post Insurance car quote automation."""
...
from helper_functions.an_post import (
    accept_cookies,
    calculate_no_claims_discount,
    click_checkbox,
    enter_and_select_first_option_from_dropdown,
    enter_and_select_from_dropdown,
    fill_date_field,
    fill_text_field,
    format_mileage,
    select_boolean_option,
...
async def run(playwright: Playwright, data):
    """Run the An Post quote journey."""
    browser = await playwright.chromium.launch(headless=False)
    page = await browser.new_page()
    await page.goto(AN_POST_QUOTE_URL)
    ...
    await fill_text_field(
        page, "Mobile Number", general_helpers.format_phone(data["phone"])
    )
    await fill_text_field(page, "Email", data["email"])
    await enter_and_select_from_dropdown(
        page, "[placeholder='Begin typing...']", data["occupation"]
    )

    # ==================== YOUR CAR SECTION ====================
    print("\n--- Filling Your Car Section ---")

    await enter_and_select_first_option_from_dropdown(
        page,
    ...
    await fill_text_field(
        page, "What is the current market value of the car?", str(data["car_value"])
    )

    # Right/Left hand drive
    await select_tile_option(
        page,
        "Is the car right or left hand drive?",
        AN_POST_MAPPINGS["right_hand_drive"][data["right_hand_drive"]],
    )

    # Registered in Ireland
    await select_boolean_option(
        page,
        "Is the car registered in Ireland?",
        AN_POST_MAPPINGS["registered_in_ireland"][data["registered_in_ireland"]],
    )

    # Is imported
    await select_boolean_option(
        page,
        "Is the car imported from outside of the Republic of Ireland?",
        AN_POST_MAPPINGS["is_imported"][data["is_imported"]],
    )

    # Car purchase date
    await fill_date_field(
        page, "When did you purchase the car?", data["car_purchase_date"]
    )

    # Annual mileage dropdown
    await select_dropdown_option(
        page,
        "How many Kilometers will the car travel over the next 12 months?",
        format_mileage(data["estimated_mileage"]),
    )
    ...
    await select_dropdown_option(
        ...
        AN_POST_MAPPINGS["registered_owner"][data["registered_owner"]],
    )
    ...
    await select_tile_option(
        ...
        AN_POST_MAPPINGS["car_usage"][data["car_usage"]],
    )
    ...
    await select_tile_option(
        ...
        AN_POST_MAPPINGS["licence_type"][data["licence_type"]],
    )
    await asyncio.sleep(0.5)

    # Licence duration dropdown
    await select_dropdown_option(
        page, "How long have you had your licence for?", data["licence_duration"]
    )

    # Penalty points
    await select_boolean_option(
        page,
        "Do you have any active penalty points?",
        AN_POST_MAPPINGS["has_penalty_points"][data["has_penalty_points"]],
    )
    ...
    await select_tile_option(
        ...
        AN_POST_MAPPINGS["driving_experience"][data["driving_experience"]],
    )
    await asyncio.sleep(0.5)
    ...
    await select_dropdown_option(
        page,
        "How many consecutive years have you held a private car policy in your own name without an accident or claim?",
        calculate_no_claims_discount(data["no_claims_discount"]),
    )
    await asyncio.sleep(0.5)
    ...
    await select_dropdown_option(
        page,
        "In what country was your most recent no claim discount earned?",
        general_helpers.capitalize_first_letter(data["country_of_most_recent_ncd"]),
    )
    await asyncio.sleep(0.5)
    ...
    await select_dropdown_option(
        ...
        general_helpers.capitalize_first_letter(data["previous_insurer"]),
    )
    await asyncio.sleep(0.5)

    # Policy start date
    await fill_date_field(
        page, "When do you require cover from", data["policy_start_date"]
    )
    await asyncio.sleep(1)
    ...
    await select_boolean_option(
        page,
        "Is this the same as the end date of your existing policy",
        AN_POST_MAPPINGS["same_as_current_policy_end_date"][
            data["same_as_current_policy_end_date"]
        ],
    )
    ...
    await select_tile_option(
        page,
        "How do you normally pay?",
        AN_POST_MAPPINGS["payment_type"][data["payment_type"]],
    )
    ...
    await select_boolean_option(
        ...
        AN_POST_MAPPINGS["add_additional_driver"][data["add_additional_driver"]],
    )
    ...
    await select_boolean_option(
        page,
        "From time to time we would like to contact you",
        AN_POST_MAPPINGS["marketing_consent"][data["marketing_consent"]],
    )
    await asyncio.sleep(1)

    await select_boolean_option(
        page,
        "Would you like to only receive details and/or special offers in relation to our Car Insurance by Email, SMS, Post or Phone? Is this ok? (You can withdraw consent at any time.)",
        AN_POST_MAPPINGS["marketing_consent"][data["marketing_consent"]],
    )
...
async def main(data):
    """Run An Post as a standalone provider."""
